...
# Sector ETFs
ETF_POOL = {
    "SMH": "半导体", "XLK": "科技", "XLE": "能源",
    "XLF": "金融",  "ARKK": "创新", "GLD": "黄金",
    "IWM": "小盘",  "QQQ": "Nasdaq100",
}

# US index
US_INDICES = {"^GSPC": "S&P500", "^IXIC": "Nasdaq"}

# Fallback pool (used if Screener fails)
US_POOL = [
    "SPCX","ASTS","LUNR","RDW","RKLB",
    "NVDA","INTC","MU","AMD","QCOM","AVGO",
    "TSLA","META","GOOGL","AMZN","AAPL","MSFT",
    "ROKU","PLTR","COIN","MSTR","HOOD","GS","JPM",
]
...
def dir_arrow(v): return "▲" if v >= 0 else "▼"
        ...
        for sym in (tickers if isinstance(tickers, list) else [tickers]):
            try:
                ...
                if len(closes) >= 2:
                    ...
                    result[sym] = {"price": curr, "pct": (curr - prev) / prev * 100}
                elif len(closes) == 1:
                    result[sym] = {"price": float(closes.iloc[-1]), "pct": 0.0}
            except Exception as e:
                print(f"  parse err {sym}: {e}")
    except Exception as e:
...
def fetch_crypto_headline():
    ...
    if tm and lm:
        return html.unescape(tm.group(1).strip()), re.sub(r'<.*', '', lm.group(1).strip())
    return None, None

def build_crypto_msg():
    prices = fetch_crypto_data()
    btc, eth = prices.get("bitcoin", {}), prices.get("ethereum", {})
    title, link = fetch_crypto_headline()

    btc_p, btc_pct = "${:,.0f}".format(btc.get("usd", 0)), btc.get("usd_24h_change", 0)
    eth_p, eth_pct = "${:,.0f}".format(eth.get("usd", 0)), eth.get("usd_24h_change", 0)
...
def get_screener_list(scr_id, count=5):
    """Fetch predefined screener list via yfinance Screener."""
    try:
        screener = yf.Screener()
        screener.set_predefined_body(scr_id)
        screener.set_count(count)
        quotes = screener.response.get("quotes", [])
        result = []
        for q in quotes[:count]:
            result.append({
                "sym":   q.get("symbol", ""),
                "name":  q.get("shortName", q.get("symbol", ""))[:15],
...
def build_us_msg():
    # Indices + ETFs
    all_syms = list(US_INDICES.keys()) + list(ETF_POOL.keys())
    ...
    # Try live screener lists
    most_active = get_screener_list("most_actives", 5)
    gainers     = get_screener_list("day_gainers",  5)
    losers      = get_screener_list("day_losers",   5)

    # Fallback if screener unavailable
    if not gainers:
        pool_q    = get_quotes_yf(US_POOL)
        pool_list = [(s, pool_q[s]) for s in US_POOL if s in pool_q and abs(pool_q[s]["pct"]) > 0.1]
        mk = lambda lst: [{"sym": s, "name": s, "price": q["price"], "pct": q["pct"], "vol": 0} for s, q in lst]
        gainers     = mk(sorted(pool_list, key=lambda x: x[1]["pct"], reverse=True)[:5])
        losers      = mk(sorted(pool_list, key=lambda x: x[1]["pct"])[:5])
        most_active = mk(sorted(pool_list, key=lambda x: abs(x[1]["pct"]), reverse=True)[:5])

    def sline(item, show_vol=False):
        p   = item["pct"]
        vol = "  vol:" + fmt_vol(item["vol"]) if show_vol and item["vol"] > 0 else ""
        return arrow(p) + " " + item["sym"] + " ${:.2f}".format(item["price"]) + " (" + fmt_pct(p) + ")" + vol

    active_lines = [sline(i, show_vol=True) for i in most_active]
    gainer_lines = [sline(i) for i in gainers]
    loser_lines  = [sline(i) for i in losers]

    etf_q    = [(s, quotes[s], ETF_POOL[s]) for s in ETF_POOL if s in quotes]
    top_etfs = sorted(etf_q, key=lambda x: x[1]["pct"], reverse=True)[:4]
    etf_lines = [arrow(q["pct"]) + " " + name + "(" + sym + ") " + fmt_pct(q["pct"])
                 for sym, q, name in top_etfs]

    if sp_pct >= 0.5:
        sentiment = "美股今日整体走强，风险偏好回升。关注成交量是否持续放大确认突破有效性，逢高注意止盈。"
    elif sp_pct <= -0.5:
        sentiment = "美股今日承压，避险情绪升温。建议减少曝险，等待回调企稳后再逐步布局。关注美联储政策走向。"
    else:
        sentiment = "美股今日震荡整理，市场等待明确催化剂。建议保持中性仓位，关注成交量异动信号。"
    ...
    ig = ("📊 美股今日盘面：" + idx_line + "\n\n"
          ...
          "📂 板块ETF：\n" + "\n".join(etf_lines) + "\n\n"
          + sentiment + "\n\n"
          "#美股 #WallStreet #涨幅榜 #跌幅榜 #成交量 #板块ETF #投资策略")

    threads = ("美股 | " + idx_line + " 📈\n"
               "涨：" + " | ".join(i["sym"] + " " + fmt_pct(i["pct"]) for i in gainers[:3]) + "\n"
               "跌：" + " | ".join(i["sym"] + " " + fmt_pct(i["pct"]) for i in losers[:3]) + "\n"
               + sentiment[:60] + "…")
...
def build_asia_msg():
    ...
    avg = sum(quotes[s]["pct"] for s in ASIA_INDICES if s in quotes) / max(len([s for s in ASIA_INDICES if s in quotes]), 1)
    if avg >= 0.5:
...
def build_my_msg():
    ...
    for sym, q in top3:
        ...
        pct  = q["pct"]
        stock_lines.append(arrow(pct) + " " + name + " RM{:.2f}".format(q["price"]) + " (" + fmt_pct(pct) + ")")
...
def main():
    ...
    for fn, label_str in [(build_us_msg, "美股"), (build_asia_msg, "亚太"), (build_my_msg, "马股")]:
        try:
            send_message(fn())
        except Exception as e:
            print(label_str + " err:", e)
            send_message("⚠️ *" + label_str + "快讯获取失败*：" + str(e)[:80])
