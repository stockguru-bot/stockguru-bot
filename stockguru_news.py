...
SECTOR_MAP   = {
    ...
    "TSLA":"新能源/电动车","META":"社交媒体/AI","GOOGL":"科技/AI",
    "MSFT":"科技/AI","AAPL":"科技/消费","AMZN":"电商/云计算",
    "PLTR":"大数据/AI","COIN":"加密/金融科技","HOOD":"金融科技",
    "GS":"投资银行","JPM":"银行","MSTR":"比特币概念",
    "ASTS":"太空通讯","RKLB":"航天","QURE":"生物科技",
}
...
STOCK_KW = ["nvidia","apple","tesla","microsoft","meta","amazon","intel","micron","sandisk","hynix",
            ...
            "partnership","deal","beats","misses","guidance","overweight","underweight"]
...
def get_sentiment(title):
    ...
    bulls = ["rise","rally","surge","soars","beat","upgrade","record","profit","partnership",
             "deal","gain","high","strong","lifted","boost","jump","exceed","buy","outperform"]
    b = sum(1 for k in bears if k in t)
    u = sum(1 for k in bulls if k in t)
    if b > u: return "bear"
    if u > b: return "bull"
    return "neutral"
...
def parse_pub_date(raw_item_xml):
    m = re.search(r'<pubDate>(.*?)</pubDate>', raw_item_xml, re.DOTALL)
...
def parse_rss(xml, max_items=10):
    """Parse RSS, drop items older than NEWS_MAX_AGE_HOURS, sort newest-first."""
    items = []
    now   = datetime.datetime.utcnow()
    for m in re.finditer(r'<item[^>]*>(.*?)</item>', xml, re.DOTALL):
        raw = m.group(1)
        pub_dt    = parse_pub_date(raw)
        age_hours = None
        ...
        if tm and lm:
            ...
            if title and len(title) > 10:
                items.append({"title":title,"link":link,"desc":desc,"age_hours":age_hours})
...
def safe_first_ticker(item):
    """Safely get the first ticker string from an item, never raises IndexError."""
    t = item.get("tickers") or []
    return t[0] if t else (item.get("sym") or "")
...
def get_screener(scr_id, count=5):
    try:
        ...
        s.set_count(count)
        return [{"sym":str(q.get("symbol","")),"name":str(q.get("shortName",q.get("symbol","")))[:20],
                 "price":float(q.get("regularMarketPrice",0)),"pct":float(q.get("regularMarketChangePercent",0)),
                 "vol":float(q.get("regularMarketVolume",0))} for q in s.response.get("quotes",[])[:count]]
    except Exception as e:
        print(f"screener {scr_id}:",e)
        return []

def fetch_yf_ticker_rss(sym, max_items=4):
    """Yahoo Finance per-ticker RSS with date filtering."""
...
def fetch_yf_news_api(sym, max_items=4):
    """yfinance Ticker.news with old/new format handling and age filter."""
    results = []
    now     = datetime.datetime.utcnow()
    try:
        ticker   = yf.Ticker(sym)
        news_raw = ticker.news
        if callable(news_raw): news_raw = news_raw()
        if not isinstance(news_raw, list): return results
        for n in (news_raw or [])[:max_items]:
            try:
                if not isinstance(n, dict): continue
                # Age check via Unix timestamp
                pts = n.get("providerPublishTime") or 0
                if pts:
                    age_h = (now - datetime.datetime.utcfromtimestamp(pts)).total_seconds()/3600
                    if age_h > NEWS_MAX_AGE_HOURS: continue
                else:
                    age_h = None
                # New nested format
                if "content" in n:
                    ...
                    pub   = str((c.get("provider") or {}).get("displayName","") or "")
                ...
                if title and link:
                    results.append({"title":title,"link":link,"desc":desc,"pub":pub,
                                    "tickers":tickers,"sym":sym,"age_hours":age_h})
    ...
    def add(batch):
        for item in batch:
            ...
            if key not in seen and len(item["title"])>15:
                seen.add(key)
                if "tickers" not in item: item["tickers"] = []
                if "pub"     not in item: item["pub"]     = ""
                if "sym"     not in item: item["sym"]     = ""
                all_items.append(item)

    # 1. Per-ticker YF RSS for each hot mover (best source)
    for sym in (hot_syms or [])[:8]:
        add(fetch_yf_ticker_rss(sym, 3))

    # 2. YF top stories RSS
...
def make_copy(item, price_ctx=None):
    title   = item["title"]
    desc    = item.get("desc","")
    tickers = item.get("tickers") or []    # guarantee list
    cat     = classify(title)
    ...
    if price_ctx and price_ctx.get("sym"):
        sym=price_ctx["sym"]; px=price_ctx.get("price",0); pct=price_ctx.get("pct",0)
        px_str = "\n💹 ${} 今日 {} ${:.2f}（{}）".format(sym,"▲" if pct>=0 else "▼",px,fmt_pct(pct))

    sym_hint = tickers[0] if tickers else (item.get("sym") or "")
    sector   = SECTOR_MAP.get(sym_hint,"")
    sect_str = "（"+sector+"板块）" if sector else ""
    desc_clip= (desc[:150]+"…") if desc else ""
    related  = " ".join("$"+str(s) for s in tickers[:3] if s)

    age_str = ""
    if age is not None:
        age_str = ("  🕐 刚刚" if age<1 else
                   "  🕐 {:.0f}小时前".format(age) if age<6 else "  🕐 今日")

    if cat=="fed":
        if "hike" in t or "raise" in t or "hawkish" in t:
            ig=("🏛️ 美联储鹰派信号来袭！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【市场影响解读】\n联储官员支持年内继续加息，直接打压市场降息预期。\n\n"
                "📊 影响链：\n利率↑ → 科技/成长股估值承压\n"
                "利率↑ → 美债收益率↑ → 美元走强 → 新兴市场资金外流\n"
                "利率↑ → 银行息差扩大 → 金融板块相对受益\n\n"
                "⚠️ 建议：适当降低科技/高估值仓位，关注黄金、金融、能源防御配置。\n\n"
                "#美联储 #加息 #FOMC #利率政策 #货币政策 #美股 #通胀 #财经快讯")
            th=("🏛️ 美联储鹰派！"+title[:55]+"\n\n"
                "加息预期升温：科技成长承压，金融能源相对受益。降低高估值仓位，关注防御资产。📉")
        elif "cut" in t or "ease" in t or "pause" in t or "dovish" in t:
            ig=("🏛️ 美联储鸽派信号！市场迎来转折！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【市场影响解读】\n降息/暂停加息预期重燃，资金重新定价风险资产。\n\n"
                "📊 影响链：\n利率↓ → 科技/成长股估值提升\n"
                "利率↓ → 美债收益率↓ → 新兴市场资金回流\n"
                "利率↓ → 房地产、消费板块受益\n\n"
                "✅ 关注：科技、生物科技、小盘成长股迎阶段性机会。"
                "关键验证：后续CPI/PCE是否持续回落。\n\n"
                "#美联储 #降息 #FOMC #利率政策 #货币政策 #美股 #风险偏好 #财经快讯")
            th=("🏛️ 美联储鸽派！"+title[:55]+"\n\n"
                "降息预期重燃，科技成长股迎布局机会。关注后续通胀数据验证降息路径。🚀")
        else:
            ig=("🏛️ 美联储最新表态 | 市场核心变量"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【为什么重要？】\n美联储政策是全球资产定价最核心变量，"
                "每次官员表态都可能触发债券、美元、股市连锁反应。\n\n"
                "📊 重点追踪：\n① 措辞变化（data dependent程度）\n"
                "② PCE/CPI通胀是否持续回落\n③ 非农数据是否出现明显降温\n\n"
                "策略：维持中性仓位，等待更明确信号后调整方向。\n\n"
                "#美联储 #FOMC #利率 #货币政策 #美股 #宏观经济 #财经快讯")
            th=("🏛️ "+title[:60]+"\n\n"
                "利率政策牵动全球市场。追踪通胀及就业数据，维持中性仓位等待信号。📊")

    elif cat=="stock":
        if sent=="bull":
            ig=("📈 "+(related+" " if related else "")+"个股重磅利好"+sect_str+"！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【利好解读】\n"
                +("分析师大幅上调目标价，机构对该股基本面信心显著提升，短期获得强力支撑。"
                  if any(k in t for k in ["target","price","lifted","raised"]) else
                  "重大催化剂事件落地，提振市场对该公司成长前景的信心。")+"\n\n"
                "📊 交易策略参考：\n"
                "✅ 成交量是关键：突破时若成交量显著放大（超20日均量），信号更可靠\n"
                "✅ 分批布局：消息驱动当日冲高后次日易出现获利回吐\n"
                "⚠️ 避免满仓追高，设好止损位\n\n"
                "#美股 #个股行情 #WallStreet #热门股 #财经快讯"
                +(" "+related if related else ""))
            th=("📈 "+(related+" " if related else "")+title[:60]+"\n\n"
                "利好催化驱动！成交量放大是突破有效性的关键确认。分批布局，避免追高。")
        elif sent=="bear":
            ig=("📉 "+(related+" " if related else "")+"个股重磅利空"+sect_str+"！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【利空解读】\n"
                +("分析师下调目标价或评级，机构对该公司基本面出现分歧，短期股价承压风险上升。"
                  if any(k in t for k in ["downgrade","cut","lower","reduce"]) else
                  "负面催化剂打击市场信心，短期买盘力量明显减弱。")+"\n\n"
                "📊 风险管理建议：\n"
                "⚠️ 持仓者：评估止损位是否合理，避免情绪化持有\n"
                "👀 观望者：等待关键支撑位（200日均线/前期低点）企稳缩量后再布局\n"
                "❌ 切忌：在下行趋势中盲目抄底\n\n"
                "#美股 #个股行情 #WallStreet #热门股 #财经快讯"
                +(" "+related if related else ""))
            th=("📉 "+(related+" " if related else "")+title[:60]+"\n\n"
            ...
            ig=("⚡ "+(related+" " if related else "")+"个股要闻"+sect_str+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【市场解读】\n此类消息往往是板块情绪变化的先行指标，"
                "需结合整体大盘方向及成交量变化综合判断实际影响力。\n\n"
                "📊 关注要点：\n① 该股在同板块中的相对强弱变化\n"
                "② 机构资金流向（大单净流入/流出）\n"
                "③ 期权市场隐含波动率是否出现异动\n\n"
                "建议持续追踪，结合技术面信号再行决策。\n\n"
        ...
        if sent=="bear":
            ig=("🔴 加密市场警报！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【行情解读】\n链上资金外流，恐慌情绪升温。"
                "当前加密市场与宏观流动性高度相关，联储政策收紧将进一步压制风险资产。\n\n"
                "📊 风险管理参考：\n"
                "⚠️ BTC需守住关键支撑位，跌破则下行空间进一步打开\n"
                "⚠️ 山寨币在BTC走弱时跌幅通常更大，仓位管理至关重要\n"
                "✅ 恐慌指数（Fear & Greed）跌入极度恐慌区（<20）往往是中长线参考信号\n\n"
                "#Bitcoin #加密货币 #BTC #ETH #熊市 #链上数据 #风险管理")
            th=("🔴 "+title[:60]+"\n\n"
                "链上资金外流，恐慌情绪升温。守住关键支撑位，山寨仓位严格管理。")
        else:
            ig=("🚀 加密市场积极信号！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【行情解读】\n链上数据回暖，机构资金回流信号出现。"
                +("ETF持续净流入叠加长线持有者增持，" if "etf" in t else "")
                +"市场情绪从恐慌区逐步向贪婪区过渡。\n\n"
                "📊 多头确认条件：\n✅ 成交量持续放大（突破需量配合）\n"
                "✅ 链上活跃地址数回升\n✅ 稳定币净流入交易所\n\n"
                "⚠️ 牛市中同样需要风险管理，分批入场，设定止盈止损区间。\n\n"
                "#Bitcoin #加密货币 #BTC #ETH #牛市 #链上数据 #ETF")
            th=("🚀 "+title[:60]+"\n\n"
                "链上资金回流！成交量放大是确认关键。分批入场，设好止盈止损区间。")
        ...
        if sent=="bear":
            ig=("⚠️ 宏观风险预警！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【宏观解读】\n全球风险情绪面临压力测试。"
                "地缘政治、贸易摩擦或经济数据走弱，均可能触发市场风险偏好快速下行。\n\n"
                "📊 避险配置参考：\n🥇 黄金（GLD/GDX）：地缘风险及通胀预期双重支撑\n"
                "📉 美债（TLT）：避险需求推动收益率下行\n"
                "💵 美元指数：风险厌恶情绪下通常走强\n\n"
                "建议降低高风险资产仓位，增加防御性配置，静待风险释放。\n\n"
                "#宏观经济 #全球市场 #避险 #黄金 #美股 #财经快讯 #投资策略")
            th=("⚠️ "+title[:60]+"\n\n"
                "宏观风险升温！关注黄金、美债避险配置。降低科技小盘仓位，等待风险释放。")
        elif sent=="bull":
            ig=("🌐 宏观利好！全球风险偏好回升！"+age_str+"\n\n"
                "📌 "+title+"\n"+(desc_clip+"\n" if desc_clip else "")+px_str+"\n\n"
                "【宏观解读】\n地缘局势缓和或经济数据超预期，为市场提供重要正面催化剂。"
                "风险偏好回升利好：周期股、新兴市场、大宗商品相关资产。\n\n"
                "📊 关注方向：\n✅ 科技/半导体：受益于需求改善预期\n"
                "✅ 能源/材料：大宗商品需求预期回升\n"
                "✅ 新兴市场（含马股）：资金回流风险资产\n\n"
                "⚠️ 单一宏观事件影响有限，需结合联储政策及流动性环境判断持续性。\n\n"
                "#宏观经济 #全球市场 #风险偏好 #美股 #新兴市场 #财经快讯")
            th=("🌐 "+title[:60]+"\n\n"
            ...
            ig=("📊 宏观财经要闻精析"+age_str+"\n\n"
                ...
                "短期市场往往过度反应或反应不足，真正的机会来自对宏观趋势的正确判断。\n\n"
                "📊 投资者需追踪：\n① 美联储政策路径（利率期货定价变化）\n"
                "② 美国经济软着陆/硬着陆概率\n③ 企业盈利预期修正方向\n"
                "④ 全球资金流向（美元/新兴市场相对强弱）\n\n"
                "保持冷静，数据说话，避免情绪化追涨杀跌。\n\n"
            ...
            th=("📊 "+title[:60]+"\n\n"
                "宏观变量是市场方向根本驱动力。追踪联储路径及企业盈利预期，数据说话。")
...
def build_crypto_msg(seq):
    ...
    news = fetch_yf_ticker_rss("BTC-USD",3)+fetch_yf_ticker_rss("ETH-USD",2)
    if not news:
        for src in CRYPTO_RSS:
            batch=fetch_rss(src,4)
            for b in batch:
                b.setdefault("tickers",[]); b.setdefault("sym","")
            news=batch
            if news: break
    if not news:
        news=[{"title":"加密市场最新动态","link":"https://cointelegraph.com",
               "desc":"","tickers":[],"sym":"","age_hours":None}]
    item=news[0]
...
def build_market_msg(seq):
    ...
    def sl(i,vol=False):
        v="  vol:"+fmt_vol(i["vol"]) if vol and i["vol"]>0 else ""
        return arrow(i["pct"])+" $"+i["sym"]+"  ${:.2f}".format(i["price"])+"  ("+fmt_pct(i["pct"])+")"+v
    ...
    if YF_AVAILABLE:
        ...
        if etf_s:
            lines.append("\n📂 *板块ETF：*\n"+"\n".join(
                arrow(q["pct"])+" "+name+"("+sym+")  "+fmt_pct(q["pct"]) for sym,q,name in etf_s))

    if YF_AVAILABLE:
        asia_q=get_quotes_yf(list(ASIA_INDICES.keys()))
        al=[arrow(asia_q[s]["pct"])+" "+name+"  "+fmt_pct(asia_q[s]["pct"])
            for s,name in ASIA_INDICES.items() if s in asia_q]
        if al: lines.append("\n🌏 *亚太指数：*\n"+"\n".join(al))

    if YF_AVAILABLE:
        my_q=get_quotes_yf(MY_TICKERS)
        klci=my_q.get("^KLSE",{}); klci_p=klci.get("pct",0)
    ...
    body="\n".join(lines)
    tail=("整体风险偏好回升，成交量扩张中的热门股值得重点追踪。合理控制仓位，做好风险管理。" if sp_pct>=0.5 else
          "市场整体承压回落，避险情绪升温。关注黄金、美债等防御资产，谨慎对待反弹行情。" if sp_pct<=-0.5 else
          "多空力量胶着，市场等待新的方向性催化剂。中性仓位为主，关注热门个股结构性机会。")
...
def build_hot_news_msg(seq, active_items):
    hot_syms=[str(i.get("sym","")) for i in (active_items or []) if i.get("sym")][:6]
    gainers =get_screener("day_gainers",5)
    losers  =get_screener("day_losers", 5)
    hot_syms+=[str(i.get("sym","")) for i in gainers+losers if i.get("sym")]
    hot_syms =list(dict.fromkeys(hot_syms))[:10]

    price_ctx_map={}
    if YF_AVAILABLE and hot_syms:
        pq=get_quotes_yf(hot_syms[:8])
        price_ctx_map={s:{"sym":s,"price":pq[s]["price"],"pct":pq[s]["pct"]} for s in pq}
    ...
    def rank(item):
        ...
        age_pen=min((item.get("age_hours") or 0)/24,1)
        return {"fed":0,"stock":1,"macro":2,"general":3,"crypto":4}.get(cat,3)+(-1 if sym_hit else 0)+age_pen
    ...
    for item in all_news:
        ...
        if not any(len(words & frozenset(s.lower().split()))>5 for s in seen):
            seen.add(item["title"]); final.append(item)
    ...
    for item in final:
        ...
        pub  =item.get("pub","")
        # ── FIXED: safe ticker extraction — no more IndexError ──
        sym  =item.get("sym","") or safe_first_ticker(item)
        ig,th=make_copy(item, price_ctx_map.get(sym))
        pub_str="  |  "+pub if pub else ""
        parts.append("━━━━ "+label+pub_str+" ━━━━\n"
                     "🔗 ["+item["title"][:70]+"]("+item["link"]+")\n\n"
                     "📸 *Instagram：*\n"+ig+"\n\n"
                     "🧵 *Threads：*\n"+th)
...
def build_my_news_msg(seq):
    ...
    for sym in ["^KLSE","1155.KL","1023.KL"]:
        news.extend(fetch_yf_ticker_rss(sym,3))
    if not news:
        for src in MY_NEWS_RSS:
            ...
            if rel:
                for r in rel:
                    r.setdefault("tickers",[]); r.setdefault("sym","")
                news=rel; break
    ...
    ig="🇲🇾 "+ig
    pub_str="  |  "+item.get("pub","") if item.get("pub") else ""
...
def main():
    ...
    try:
        ...
        print("Crypto err:",e); send_message("⚠️ 加密快讯获取失败："+str(e)[:80])
    ...
    if YF_AVAILABLE:
        try:
            ...
            print("Market err:",e); send_message("⚠️ 市场数据获取失败："+str(e)[:80])

    try:
        msg=build_hot_news_msg(seq,active_list)
        if msg: send_message(msg); seq+=1
    except Exception as e:
        print("Hot news err:",e); send_message("⚠️ 热门要闻获取失败："+str(e)[:80])
