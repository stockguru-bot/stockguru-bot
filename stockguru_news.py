...
NEWS_MAX_AGE_HOURS = 48   # discard anything older than this
...
def parse_pub_date(raw_item_xml):
    """Extract and parse <pubDate> from a raw RSS <item> block. Returns datetime (UTC) or None."""
...
def parse_rss(xml, max_items=10):
    """Parse RSS → list of {title, link, desc, pub_dt, age_hours}.
    Items older than NEWS_MAX_AGE_HOURS are silently dropped."""
    items = []
    ...
    # Sort newest-first (items with known age first)
...
def fetch_yf_ticker_news(sym, max_items=4):
    """Yahoo Finance per-ticker RSS — includes pubDate for freshness filtering."""
...
def fetch_yf_news_api(sym, max_items=4):
    """yfinance Ticker.news — handles both old and new API formats, filters by age."""
    results = []
    now     = datetime.datetime.utcnow()
    try:
        ...
        for n in (news_raw or [])[:max_items]:
            try:
                if not isinstance(n, dict):
                    continue
                # Age check via providerPublishTime (Unix timestamp)
                pts = n.get("providerPublishTime") or 0
                if pts:
                    age_hours = (now - datetime.datetime.utcfromtimestamp(pts)).total_seconds() / 3600
                    if age_hours > NEWS_MAX_AGE_HOURS:
                        continue
                else:
                    age_hours = None
                ...
                if "content" in n:
                    ...
                    # Also check pubDate in content
                    pd_str = c.get("pubDate","")
                    if pd_str and age_hours is None:
                        try:
                            dt = parsedate_to_datetime(pd_str)
                            age_hours = (now - datetime.datetime(*dt.utctimetuple()[:6])).total_seconds()/3600
                            if age_hours > NEWS_MAX_AGE_HOURS:
                                continue
                        except Exception:
                            pass
...
def gather_stock_news(hot_syms):
    ...
    # 1. Per-ticker YF RSS for each hot mover
    for sym in hot_syms[:8]:
        add(fetch_yf_ticker_news(sym, 3))
    ...
    if cat == "fed":
        if "hike" in t or "raise" in t or "hawkish" in t:
            ig = ("🏛️ 美联储鹰派信号来袭！" + age_str + "\n\n"
                  ...
                  "利率↑ → 银行息差扩大 → 金融板块相对受益\n\n"
                  "⚠️ 建议：适当降低科技/高估值仓位，关注黄金、金融、能源等防御配置。\n\n"
            ...
            ig = ("🏛️ 美联储鸽派信号！市场迎来转折！" + age_str + "\n\n"
                  ...
                  + px_str + "\n\n"
                  "【市场影响解读】\n"
                  "降息或暂停加息预期重燃，资金重新定价风险资产。\n\n"
                  "📊 影响链：\n"
                  "利率↓ → 融资成本↓ → 科技/成长股估值提升\n"
                  "利率↓ → 美债收益率↓ → 新兴市场资金回流\n"
                  "利率↓ → 房地产、消费板块受益\n\n"
                  "✅ 关注：科技、生物科技、小盘成长股迎阶段性机会。"
                  "关键验证：后续CPI/PCE数据是否持续回落。\n\n"
        ...
        if sent == "bull":
            ig = ("📈 " + (related + " " if related else "") + "个股重磅利好" + sect_str + "！" + age_str + "\n\n"
                  ...
                  + ("分析师大幅上调目标价，机构对该股基本面信心显著提升，短期获得强力支撑。"
                     if "target" in t or "price" in t or "lifted" in t or "raised" in t else
                     "重大催化剂事件落地，提振市场对该公司成长前景的信心。")
                  + "\n\n"
                  "📊 交易策略参考：\n"
                  "✅ 成交量是关键：突破时若成交量显著放大（超20日均量），信号更可靠\n"
                  "✅ 分批布局：消息驱动行情当日冲高后次日易出现获利回吐\n"
                  "⚠️ 风险：避免满仓追高，设好止损位\n\n"
            ...
            ig = ("📉 " + (related + " " if related else "") + "个股重磅利空" + sect_str + "！" + age_str + "\n\n"
                  ...
                  + ("分析师下调目标价或评级，机构对该公司基本面出现分歧，短期股价承压风险上升。"
                     if "downgrade" in t or "cut" in t or "lower" in t else
                     "负面催化剂打击市场信心，短期买盘力量明显减弱。")
                  + "\n\n"
                  "📊 风险管理建议：\n"
                  "⚠️ 持仓者：评估止损位是否合理，避免情绪化持有\n"
                  "👀 观望者：等待在关键支撑位（200日均线/前期低点）企稳缩量后再考虑布局\n"
                  "❌ 切忌：在下行趋势中盲目抄底\n\n"
                  "#美股 #个股行情 #WallStreet #热门股 #财经快讯"
                  + (" " + related if related else ""))
            th  = ("📉 " + (related + " " if related else "") + title[:60] + "\n\n"
        ...
        if sent == "bear":
            ig = ("🔴 加密市场警报！" + age_str + "\n\n"
                  ...
                  "✅ 时机参考：恐慌指数（Fear & Greed）跌入极度恐慌区（<20）往往是中长线信号\n\n"
                  "#Bitcoin #加密货币 #BTC #ETH #熊市 #链上数据 #风险管理")
            th  = ("🔴 " + title[:60] + "\n\n"
                   "链上资金外流，恐慌情绪升温。守住关键支撑位是关键，山寨仓位严格管理。")
        else:
            ig = ("🚀 加密市场积极信号！" + age_str + "\n\n"
                  "📌 " + title + "\n"
                  + (desc_clip + "\n" if desc_clip else "")
...
def build_crypto_msg(seq):
    ...
    news = fetch_yf_ticker_news("BTC-USD", 3) + fetch_yf_ticker_news("ETH-USD", 2)
    if not news:
...
def build_hot_news_msg(seq, active_items):
    ...
    def rank(item):
        ...
        age_pen = min((item.get("age_hours") or 0) / 24, 1)  # slight penalty for older news
        return {"fed":0,"stock":1,"macro":2,"general":3,"crypto":4}.get(cat, 3) + boost + age_pen
...
def build_my_news_msg(seq):
    ...
    for sym in ["^KLSE","1155.KL","1023.KL"]:
        news.extend(fetch_yf_ticker_news(sym, 3))
    if not news:
