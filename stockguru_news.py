...
Pipeline:
  1. Collect 50-80 articles from 6 sources (no topic keyword filter)
  2. Score each by: trending ticker match + screener match + source quality + freshness
  3. Deduplicate (content similarity)
  4. Show top 4-5 articles per broadcast
"""
...
SECTOR   = {
    ...
    "GS":"投行","JPM":"银行","BAC":"银行","MSTR":"比特币",
    "ASTS":"太空通讯","RKLB":"航天","MRVL":"半导体","ORCL":"云计算",
    "SNDK":"存储/AI","QURE":"生物科技","LUV":"航空","DAL":"航空",
    "XOM":"能源","CVX":"能源","V":"支付","MA":"支付",
}
...
# News sources — ordered by quality/reliability
NEWS_SOURCES = [
    # (url, label, max_items, score_bonus)
    ("https://finance.yahoo.com/rss/topstories",              "Yahoo Finance", 30, 2),
    ("https://feeds.reuters.com/reuters/businessNews",        "Reuters",       20, 3),
    ("https://www.cnbc.com/id/100003114/device/rss/rss.html", "CNBC Markets",  15, 3),
    ("https://www.cnbc.com/id/19854910/device/rss/rss.html",  "CNBC Tech",     15, 3),
    ("https://feeds.marketwatch.com/marketwatch/topstories/", "MarketWatch",   15, 2),
    ("https://www.investing.com/rss/news.rss",                "Investing.com", 15, 2),
]
...
def parse_rss(xml, max_items=30, label="", score_bonus=0):
    ...
    for m in re.finditer(r'<item[^>]*>(.*?)</item>', xml, re.DOTALL):
        ...
        if title and len(title) > 10:
            items.append({"title":title,"link":link,"desc":desc,
                          "age_hours":age,"source":label,"score_bonus":score_bonus})
...
def fetch_rss(url, max_items=30, label="", bonus=0):
    try:
        xml = fetch(url)
        return parse_rss(xml, max_items, label, bonus)
    except Exception as e:
        print(f"RSS fail [{label}] {url[:60]}: {e}"); return []

# ── Finviz news (server-rendered HTML, aggregates many sources) ───────────────
def fetch_finviz():
    try:
        content = fetch("https://finviz.com/news.ashx")
        items = []; now = datetime.datetime.utcnow()
        today = now.strftime("%b-%d-%y")
        # Finviz table rows: <td class="news_date-cell">time</td><td><a href="url">title</a>
        for m in re.finditer(
            ...
            # Parse age: format is either "HH:MM AM/PM" (today) or "Mmm-DD-YY"
            age = None
...
# ── Generic article detector (no keyword filter, just title pattern) ───────────
GENERIC_PATTERNS = [
    r"^\d+ stocks? to watch","^top \d+ stocks?","^stocks? (rising|falling|trending)",
    r"^(what|which) stocks?","^morning brief","^market wrap","^pre-?market",
    r"^week ahead","^today'?s (top|market)","^biggest movers",
]
...
def collect_all_news(trending_syms, screener_syms):
    """
    Collect 50-80 articles from 6 sources.
    No keyword-based topic filter — volume and source quality determine selection.
    """
    ...
    # Primary: Yahoo Finance top stories (most curated popular news)
    ...
    # Finviz aggregates news from 50+ sources (server-rendered HTML)
    add(fetch_finviz())
    ...
    # Per-ticker RSS only for well-known trending tickers (not small caps)
    # Only fetch for tickers that are in SECTOR dict (major known stocks)
    ...
    print(f"Collected {len(all_items)} unique articles before scoring")
    return all_items

def score_article(item, trending_syms, screener_syms):
    """
    Score purely on market signals — no topic keywords.
    Lower score = shown first.
    """
    title = item["title"].lower()
    age   = item.get("age_hours") or 12

    # Base: source quality bonus (Reuters/CNBC higher than generic)
    score = 10 - item.get("score_bonus", 0)

    # Boost: article mentions a currently trending ticker
    trending_hit = any(s.lower() in title or s.lower().replace("-","") in title
                       for s in trending_syms if s)
    if trending_hit: score -= 3

    # Boost: article mentions a screener hot mover
    screener_hit = any(s.lower() in title for s in screener_syms if s)
    if screener_hit: score -= 2

    # Boost: article about a major known stock (in SECTOR dict)
    major_hit = any(name.lower().split("/")[0] in title
                    or sym.lower() in title
                    for sym, name in SECTOR.items())
    if major_hit: score -= 1

    # Freshness (0-3 hours = 0 penalty, older = slight penalty)
    score += min(age / 8, 2.5)
...
def deduplicate(items):
    ...
    for item in items:
        ...
        # If 60%+ of words overlap with an already-kept article, skip
        is_dup = False
...
def sentiment(title):
    ...
    neg = sum(1 for w in ["fall","drop","crash","slump","miss","plunge","slips","decline",
                           ...
                           "fine","lawsuit","probe","ban","halt","concern"] if w in t)
...
def topic(title, sym=""):
    t = title.lower()
    if any(k in t for k in ["bitcoin","btc","ethereum","eth","crypto","blockchain","defi"]): return "crypto"
    if any(k in t for k in ["federal reserve","fomc","rate cut","rate hike","monetary policy",
                              "interest rate","powell","warsh","fed chair","basis points"]): return "fed"
    if any(k in t for k in ["war","military","sanction","conflict","attack","invasion",
                              "missile","ceasefire","iran","russia invades","taiwan strait",
                              "north korea","airstrike","nuclear","troops","pentagon"]): return "geo"
    if sym or any(k in t for k in ["earnings","buyback","acquisition","ipo","merger",
                                    "target raised","upgrade","downgrade","revenue","profit",
                                    "partnership","price target","beats","misses"]): return "stock"
    if any(k in t for k in ["gdp","jobs","unemployment","cpi","pce","recession","tariff",
                              "oil price","gold","inflation","trade","deficit","yields"]): return "macro"
    return "general"
...
def build_hot_news(seq, active, gainers, losers):
    """
    Large-pool news selection:
    Collect 50-80 articles from Yahoo Finance, Reuters, CNBC, MarketWatch, Finviz
    Score by market signals, deduplicate, show top 4-5.
    """
    trending = get_trending()
    screener_syms = [i["sym"] for i in active+gainers+losers if i.get("sym")]
    all_syms = list(dict.fromkeys(trending + screener_syms))
    ...
    # Pick top 4 (skip generic ones, ensure variety of sources)
    final = []
    sources_used = set()
    for item in final_pool:
        if len(final) >= 4: break
        src = item.get("source","")
        # Allow max 2 articles per source for variety
    ...
    # Get price context for any known tickers mentioned
    all_known = list(set(s for item in final for s in SECTOR if s.lower() in item["title"].lower()))
    pq = quotes(all_known[:8]) if YF_OK and all_known else {}
